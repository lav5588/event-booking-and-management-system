...
import Skeleton  from '../material-ui-services/EventSkeleton';
import ModalConfirm from '../material-ui-services/ModalConfirm';
...
const Events = () => {
  ...
  return (
    <>
          ...
          eventData?.map((data,index) => {
            return (
              <div key={data._id} className='p-2.5 m-2.5 w-96  shadow-md bg-white  rounded-lg hover:shadow-gray-900 hover:scale-105 hover:shadow-2xl'  >
                ...
                <button className={`text-3xl relative bg-white rounded-[50%] p-3 top-[-15rem] left-[20rem] ${data.isLike ? "text-[red]" : "text-[#969090]"} hover:text-[red]`} ref={(el)=>{favoriteButtonRef.current[index]=el}} onClick={() => handleFavorites(data._id)}><FaHeart className='' /></button>
      ...
      {modalOpen && <ModalConfirm buttonRef={favoriteButtonRef} title={"You are not Logged In"} message={"Do you want to log in ?"} okHandler={navigateToSignIn}/>}
      </>
  )
}
